RSpec.describe DiscoveryEngine::Query::Search do
  subject(:search) { described_class.new(query_params) }
  ...
  describe "#result_set" do
    context "when the search is successful" do
      ...
      context "when sorting by ascending title" do
        let(:query_params) { { q: "garden centres", order: "title" } }

        it "calls the client with the expected parameters" do
          expect(client).to have_received(:search).with(
            hash_including(order_by: "title"),
          )
        end
      end

      context "when sorting by descending title" do
        let(:query_params) { { q: "garden centres", order: "-title" } }

        it "calls the client with the expected parameters" do
          expect(client).to have_received(:search).with(
            hash_including(order_by: "title desc"),
          )
        end
      end
